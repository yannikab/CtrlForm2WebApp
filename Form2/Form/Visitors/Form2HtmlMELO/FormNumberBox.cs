...
namespace Form2.Form.Visitors
{
    public partial class Form2HtmlMELOVisitor
    {
        public virtual void Visit(FormNumberBox formNumberBox, HtmlContainer htmlContainer)
        {
            ...
            if (!string.IsNullOrWhiteSpace(formNumberBox.CssClass))
                htmlDiv.Class.AddRange(formNumberBox.CssClass.Split(' ').Where(s => s != string.Empty));
            ...
            if (!initialize && firstInvalidId == null)
                if (formNumberBox.IsRequired && !formNumberBox.HasValue || !formNumberBox.IsValid)
                    firstInvalidId = htmlNumberBox.Id.Value;
